...
| 子任務型態 | model | 理由 |
|-----------|-------|------|
| 搜尋/定位（找檔案、找定義） | `sonnet`（Explore 預設即可） | 搜索靠工具不靠推理 |
...
| 超大量讀取（>50 檔） | `sonnet[1m]` | 用大 context 換少分批 |
...
- **解出模式後降級**：`opus`/`sonnet` 解出的做法，整理成「模式＋範例」後改派
  `haiku` 批次套用剩餘案例。
- **同一件事最多重試 2 輪**（不分模型累計）。第 3 輪前必須先過
...
- Task Card 的 `allowed_tools` 含 `subagent_dispatch` 才可派工（新建卡時記得加）。
- 派工不豁免三條硬規則：subagent 一樣不得對外發送、不得刪除（PERMISSIONS deny 對全體生效，
  `scripts/permissions_guard.py` hook 攔 Bash 層）。
