...
### 批次 web-search 節流與快取（2026-06-25 T07 新增，補 T04 缺口 #3）
單卡 ≤3 輪是「事後上限」；同批／同日多卡另加事前節流，避免重演 2026-04 S01（第 3 輪 web search 撞 rate limit）：
- **快取優先**：外查前先查 project 內部與既有 `outputs/`、`memory/`（同主題前卡的結果可重用）。能複用就不重查——延伸既有「能讀檔解決的不做 web search」。
- **批次合計上限**：同一批次（如 test_batch 系列或同日多卡）web search 合計建議 ≤ 卡數 × 2 輪；逼近時改以既有結果 + 自身知識補足。
- **卡間間隔**：多卡連續執行時，web search 之間留間隔，不在短時間集中呼叫，降低 rate-limit 機率。
- 觸發上限時：以既有搜尋結果 + 內部資料補足缺口，缺口明確標 [待驗證]，不硬重試。
