...
<<<<<<< HEAD
`skill_type` 請使用：`research` / `analysis` / `writing` / `ops` / `review`。參考 `tasks/examples/` 下的範例（例如 `2026-04-21_decision-feasibility-analysis-example.yaml` 可用於決策比較／可行性評估）。
=======
`skill_type` 請使用：`research` / `analysis` / `writing` / `ops` / `review`。參考 `tasks/examples/` 下的範例。
`skill_type` 允許值以驗證腳本與 schema 為準。
>>>>>>> origin/codex/update-execution_log_schema.yaml-skill_type-comment
