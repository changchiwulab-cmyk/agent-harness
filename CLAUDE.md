...
## Runtime 強制（v2.1，可機械驗證）

四層 Gate 與硬規則 #2/#3 已可執行，不再純靠自律（強制力分級見 README「Runtime 強制力」）：
- `python scripts/gate_runner.py --run-log <log>`：對任務獨立重跑四層 Gate（schema 重用 validate_task_card.py）。
- `python scripts/failure_tracker.py record-failure|check`：連續失敗計數，達 3 次寫 logs/errors/ 並 exit 1 強制停。
- PreToolUse `scripts/write_guard.py`：攔 system//skills//memory//outputs/reports//CLAUDE.md（ask 級）寫入；核准後設 `HARNESS_WRITE_GUARD_OVERRIDE=1` 放行。
- 硬規則 #1（無 Task Card 不執行）仍為 prompt 級，無法在檔案寫入時可靠強制。
