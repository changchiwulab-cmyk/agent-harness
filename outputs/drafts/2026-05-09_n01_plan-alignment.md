...
2. **N2 — 修正 audit 計數錯誤歸因（已執行）**
   - 發現：A01/W01 都把「30+」歸因 README，但 grep 顯示 README 從無此字樣；真正出處是 plan §Context + A01 task card 自身 context（引自 plan）。
   - 真實計數：本 PR 前 18 筆 audit（grep `^- task_id:` 扣除格式範例 1 筆），本 PR 後 23 筆。
   - 修正範圍：A01 §1.1 + §10、W01 §1.4 + 第 11 章 + §9、N1 §7.2（即此段）。
   - 不修：plan、A01 task card context 欄位 — 視為歷史 snapshot；README 也無需動（本來就沒寫）。
