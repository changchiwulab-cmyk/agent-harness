...
<!-- AUTO_AUDIT_BEGIN -->
```yaml
task_id: 20260704-F01
date: '2026-07-04'
skill_type: ops
goal: 把 Fable 5 session 的判斷力外化成長期制度檔案：診斷、CLAUDE.md 路由化重寫、模型調度守則、判斷 rubric、派工模板、維護協議、交接信
status: done
risk_level: medium
...
result_summary: 13 個 DoD 全數完成。新增 6 個 system/ 制度檔+verifier agent+2 個 memory 種子檔+診斷報告；CLAUDE.md
  ...
  [1m] 別名不可用於派工。CI 全綠（spec/budget/YAML/manifest/hooks/audit/e2e×2/revisit）。session
  中斷 2 次靠 checkpoint 無損接續，教訓已記 lessons.md L-20260705-01。
