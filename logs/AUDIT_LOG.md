...
result_summary: DoD 5/5。佈線擇定 Stop hook（優先案成立）：Stop payload 雖無改動檔清單，但以 git 推導（工作樹 dirty
  ...
  次選的 fallback 條件不成立；CI 亦看不到未 commit 的 session 草稿、警告離工作現場太遠。advisory 語意以測試鎖定：命中未隔離注入樣式→stderr（檔案路徑＋命中規則）＋systemMessage＋ledger，永遠
  exit 0 不 block、乾淨時安靜、[未受信任來源] 區塊級隔離放行。誤報觀察機制：tracked ledger logs/untrusted_content_hits.jsonl（ephemeral
